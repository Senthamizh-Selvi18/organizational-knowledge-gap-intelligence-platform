...
public class CommunityGroupServiceImpl implements CommunityGroupService {
    ...
    public List<CommunityGroupResponseDTO> listAllGroups() {
        ...
        return communityGroupRepository.findAllByOrderByCreatedAtDesc()
                .stream().map(g -> toDto(g, employee)).collect(Collectors.toList());
    }
    ...
    public void leaveGroup(Long groupId) {
        ...
        GroupMembership membership = groupMembershipRepository.findByGroupAndMember(group, employee)
    ...
    public List<GroupPostResponseDTO> listPosts(Long groupId) {
        CommunityGroup group = findGroup(groupId);
        return groupPostRepository.findByGroupOrderByCreatedAtDesc(group)
    ...
    public GroupPostResponseDTO createPost(Long groupId, GroupPostRequestDTO request) {
        ...
        notifyOtherMembers(group, employee,
    ...
    public void deletePost(Long groupId, Long postId) {
        ...
        GroupPost post = groupPostRepository.findById(postId)
                .orElseThrow(() -> new GroupPostNotFoundException("Post not found with id: " + postId));

        if (!post.getGroup().getId().equals(group.getId())) {
            throw new GroupPostNotFoundException("Post not found in this group.");
        }
    ...
    public List<GroupEventResponseDTO> listEvents(Long groupId) {
        CommunityGroup group = findGroup(groupId);
        return groupEventRepository.findByGroupOrderByEventDateTimeAsc(group)
                .stream().map(this::toEventDto).collect(Collectors.toList());
    }

    @Override
    @Transactional
    public GroupEventResponseDTO createEvent(Long groupId, GroupEventRequestDTO request) {
        CommunityGroup group = findGroup(groupId);
        Employee employee = currentEmployee();
        requireMember(group, employee);
        ...
        notifyOtherMembers(group, employee,
                ...
                employeeName(employee) + " scheduled \"" + request.getTitle() + "\".");
    ...
    public void deleteEvent(Long groupId, Long eventId) {
        ...
        GroupEvent event = groupEventRepository.findById(eventId)
                .orElseThrow(() -> new GroupEventNotFoundException("Event not found with id: " + eventId));

        if (!event.getGroup().getId().equals(group.getId())) {
            throw new GroupEventNotFoundException("Event not found in this group.");
        }

        requireAuthorOrGroupOwnerOrAdmin(group, event.getCreatedBy());
        groupEventRepository.delete(event);
    }

    // ---------- helpers ----------

    private void notifyOtherMembers(CommunityGroup group, Employee actor, String title, String message) {
        List<GroupMembership> members = groupMembershipRepository.findByGroupOrderByJoinedAtAsc(group);
        for (GroupMembership m : members) {
            Employee memberEmployee = m.getMember();
            if (memberEmployee.getId().equals(actor.getId())) continue;
    ...
    private CommunityGroup findGroup(Long groupId) {
        return communityGroupRepository.findById(groupId)
                .orElseThrow(() -> new CommunityGroupNotFoundException("Group not found with id: " + groupId));
    }

    private void requireMember(CommunityGroup group, Employee employee) {
        if (!groupMembershipRepository.existsByGroupAndMember(group, employee)) {
            throw new AccessDeniedException("You must join this group first.");
        }
    }

    private void requireOwnerOrAdmin(CommunityGroup group) {
        Employee employee = currentEmployee();
        boolean isOwner = group.getCreatedBy().getId().equals(employee.getId());
        if (!isOwner && !isSystemAdmin()) {
            throw new AccessDeniedException("Only the group owner can do this.");
        }
    }

    private void requireAuthorOrGroupOwnerOrAdmin(CommunityGroup group, Employee author) {
        Employee employee = currentEmployee();
        boolean isAuthor = author.getId().equals(employee.getId());
        boolean isGroupOwner = group.getCreatedBy().getId().equals(employee.getId());
    ...
    private String employeeName(Employee employee) {
        return employee.getUser() != null ? employee.getUser().getName() : "Someone";
    }

    private CommunityGroupResponseDTO toDto(CommunityGroup group, Employee viewer) {
        ...
        dto.setCreatedByName(employeeName(group.getCreatedBy()));
        dto.setMemberCount(groupMembershipRepository.countByGroup(group));
        dto.setMember(groupMembershipRepository.existsByGroupAndMember(group, viewer));
        dto.setCreatedAt(group.getCreatedAt());
        return dto;
    }

    private GroupMemberResponseDTO toMemberDto(GroupMembership membership) {
    ...
    private GroupPostResponseDTO toPostDto(GroupPost post) {
    ...
    private GroupEventResponseDTO toEventDto(GroupEvent event) {
        GroupEventResponseDTO dto = new GroupEventResponseDTO();
        ...
        dto.setCreatedByName(employeeName(event.getCreatedBy()));
